...
try:
    ...
    from chordlib._internal import dsp, utils
    ...
    class Dummy:
        ...
        def detect(self, *args, **kwargs):
            return np.array([0, 1, 2]), ["C", "G", "Am"], [11.2, 10.8, 10.1], None

    ChordsDetectionBeats = Dummy

    def extract_pcp(y, sr, n_fft, hop_length, **kwargs):
        return np.random.rand(int(len(y) / hop_length), 12)

    def track_beats(y, sr, hop_length, sparse, **kwargs):
        return 0, np.linspace(0, int(len(y) / hop_length) - 1, 20).astype(int)
    ...
    def group_chord_events(times, chords, strengths, min_duration_sec=0.1):
        ...
        times, chords, strengths = (
            np.array(times)[sorted_indices],
            np.array(chords)[sorted_indices],
            np.array(strengths)[sorted_indices],
        )
    ...
    class dsp:
        @staticmethod
        def compute_onset_strength(y, sr, hop_length, **kwargs):
            return np.random.rand(int(len(y) / hop_length))

    class utils:
        @staticmethod
        def frames_to_time(frames, sr, hop_length, **kwargs):
...
class AnalysisWorker(QThread):
    ...
    def run(self):
        try:
            ...
            onset_env = dsp.compute_onset_strength(
                y=y, sr=sr, hop_length=self.hop_length, aggregate=np.median
            )
            _, beats = track_beats(y=y, sr=sr, hop_length=self.hop_length, sparse=True)
            results["onset_envelope"] = onset_env
            results["beats"] = beats
            self.progress.emit("2/3: Extracting PCP Chromagram...", 60)
            pcp_sequence = extract_pcp(
                y, sr, n_fft=self.n_fft, hop_length=self.hop_length
            )
            ...
            grouped_chords = group_chord_events(
                np.array(times),
                chords,
                strengths,
