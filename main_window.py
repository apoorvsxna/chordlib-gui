...
class MainWindow(QMainWindow):
    ...
    def _get_chord_templates(self):
        ...
        for i in range(12):
            ...
            if note_names_sharp[i] != note_names_flat[i]:
                templates[note_names_flat[i]] = maj_template
            min_template = np.roll(self.chord_matcher._profile_m, i)
            templates[note_names_sharp[i] + "m"] = min_template
            if note_names_sharp[i] != note_names_flat[i]:
                templates[note_names_flat[i] + "m"] = min_template
        return templates
    ...
    def open_file(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open Audio File", "", "Audio Files (*.wav *.mp3 *.flac *.aac)"
        )
        if file_path:
            ...
            self.status_bar.showMessage(
                f"Loaded '{os.path.basename(file_path)}'. Ready to analyze."
            )
    ...
    def open_settings_dialog(self):
        dialog = SettingsDialog(
            self, self.n_fft, self.hop_length, self.min_duration_sec
        )
        if dialog.exec_() == SettingsDialog.Accepted:
            settings = dialog.get_values()
            self.n_fft, self.hop_length, self.min_duration_sec = settings.values()
            self.status_bar.showMessage(
                f"Settings updated: N_FFT={self.n_fft}, Hop={self.hop_length}, MinDur={self.min_duration_sec}s"
            )
    ...
    def export_chords(self):
        ...
        default_path = os.path.join(
            os.path.dirname(self.current_file_path), f"{base_name}_chords"
        )
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"Chords successfully exported to {os.path.basename(filePath)}"
            )
    ...
    def handle_media_status_change(self, status):
        if status == QMediaPlayer.LoadedMedia:
            ...
            self.status_bar.showMessage("Error: Could not load the audio file.")
        elif status == QMediaPlayer.StalledMedia:
            self.status_bar.showMessage("Playback stalled, buffering...")
    ...
    def update_position(self, position):
        ...
        self.time_label.setText(
            f"{pos_secs//60:02d}:{pos_secs%60:02d} / {dur_secs//60:02d}:{dur_secs%60:02d}"
        )
    ...
    def seek_from_plot_click(self, event):
        if event.button() == Qt.LeftButton and self.analysis_results:
            ...
            if 0 <= time_s <= duration_s:
                self.media_player.setPosition(int(time_s * 1000))
    ...
    def _find_chord_index_at_time(self, time_s):
        if "chord_start_times" not in self.analysis_results:
            return -1
        ...
        if 0 <= index < len(chords) and time_s < chords[index]["end"]:
            return index
        return -1
    ...
    def update_auto_scroll(self, current_time_s):
        ...
        target_center = None
        if current_time_s > dead_zone_right:
            target_center = current_time_s - view_width * 0.2
        elif current_time_s < dead_zone_left:
            target_center = current_time_s + view_width * 0.2
