...
class VisualsManager:
    ...
    def _draw_chromagram(self, results):
        ...
        chromagram_cens = librosa.feature.chroma_cens(
    ...
    def populate_chord_table(self, results):
        ...
        for i, event in enumerate(results.get("grouped_chords", [])):
            ...
            self.win.chord_table.setItem(
                i, 0, QTableWidgetItem(f"{event['start']:.2f}")
            )
            ...
            self.win.chord_table.setItem(
                i, 3, QTableWidgetItem(f"{confidence_pct:.1f}%")
            )

    def draw_chord_timeline(self, results):
        for item in self.chord_viz_items:
            self.win.p_timeline.removeItem(item)
        ...
        for event in results.get("grouped_chords", []):
            ...
            text = pg.TextItem(
                text=chord_name, color=(255, 255, 255), anchor=(0.5, 0.5)
            )
    ...
    def update_live_pcp(self, time_s, results):
        ...
        if 0 <= frame_index < len(pcp_sequence):
            ...
            self.win.current_pcp_display = (
                1 - INTERPOLATION_FACTOR
            ) * self.win.current_pcp_display + INTERPOLATION_FACTOR * target_pcp
    ...
    def update_table_highlight(self, chord_index):
        if chord_index >= 0 and self.win.chord_table.currentRow() != chord_index:
            ...
            self.win.chord_table.scrollToItem(
                self.win.chord_table.item(chord_index, 0),
